...
1. **Pegar a issue** (orquestrador): se a pessoa não deu o identifier, usar o subagent `linear-triage` para escolher; senão, `linear get --id <IDENTIFIER>` para ler título, descrição e `branchName`.
...
4. **Implementar** (executor — subagent): o subagent `linear-implement` recebe o path do arquivo de plano, cria a worktree a partir do `branchName`, executa o plano e edita o código. Este agente só implementa.
