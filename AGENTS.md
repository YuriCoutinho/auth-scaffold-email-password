...
* `src/plugins/app/auth/` guarda a identidade e decora `fastify.auth` no `index.ts`. `repository.ts` é a interface `AuthRepository`, `drizzle-repository.ts` é o adaptador Drizzle dela, `create-auth.ts` monta os fluxos, `signup.ts`, `resend-code.ts`, `verify-code.ts`, `login.ts` e `authenticate.ts` são os services, `send-signup-code.ts` é o envio de código que `signup` e `resend-code` compartilham, e `emails/` guarda os templates
* `src/plugins/app/sessions/` guarda a sessão e decora `fastify.sessions` no `index.ts`. `repository.ts` é a interface `SessionRepository`, `drizzle-repository.ts` é o adaptador Drizzle dela e também aceita uma transação aberta fora, que é o que mantém o autologin do cadastro atômico, `create-sessions.ts` monta os fluxos, e `logout.ts`, `logout-all.ts` e `list-sessions.ts` são os services. A dependência é de mão única: `auth` pode importar de `sessions`, e nenhum arquivo de `sessions` importa de `auth`
