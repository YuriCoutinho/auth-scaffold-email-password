...
`POST /auth/forgot-password` responde `202` com uma mensagem fixa e com o cookie setado em todos os casos. Quando o endereço não pertence a conta nenhuma, nada é gravado, nenhum email sai, e o cookie recebe um token descartável, gerado com a mesma função e a mesma entropia do token real. A resposta é byte a byte idêntica à de um endereço conhecido.
