...
* **Migration inicial regenerada** via Drizzle Kit (`db:generate`): sem banco em produção, a `0000` é reescrita a partir do schema novo em vez de criar migration incremental (bancos dev locais são recriados)
