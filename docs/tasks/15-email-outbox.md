...
Uma linha abandonada segue o caminho inverso, porque as duas partes dela têm prazos diferentes. O conteúdo é limpo no instante em que ela é abandonada, seja pela desistência, pelo vencimento ou pelo cancelamento, já que uma mensagem que nunca mais será enviada é só exposição. O resto da linha, ou seja, o tipo, o destinatário, o número de tentativas e a mensagem do último erro, sobrevive por trinta dias, porque é disso que se precisa para investigar uma falha de entrega. O destinatário fica junto por um motivo simples: investigar uma falha sem saber quem foi afetado não serve para nada. Os três estados terminais que não são entrega, ou seja, `failed`, `expired` e `canceled`, gravam em `next_attempt_at` o instante da transição e por isso são medidos por essa mesma janela, com uma única cláusula de expurgo.
...
### Nenhuma mensagem entrega um código que já não vale

A política de repetição sozinha não cobre o problema que ela parece cobrir, e vale ser explícito sobre o furo. Desistir é contar tentativas, e tentativa só acontece enquanto o worker está de pé. Se ele fica fora do ar por um deploy, uma queda ou um contêiner reagendado, as linhas ficam pendentes com o contador congelado, e quando ele volta reivindica todas elas e entrega sem que nada tenha olhado a idade do que está entregando. A coluna `created_at` existia desde o começo e nunca era lida por ninguém.

Duas causas diferentes levam ao mesmo sintoma. A primeira é o código que morreu de velho, porque ele vale quinze minutos e uma linha parada mais tempo que isso entrega algo que a confirmação vai recusar. A segunda é o código que foi substituído, porque cada reenvio cunha um novo código e invalida o anterior no ato da gravação, e não quando ele expira, de forma que as linhas ainda pendentes de antes do reenvio carregam códigos já mortos. Nessa segunda, um worker que volta de uma indisponibilidade dispara uma rajada de emails em que nenhum código funciona. A regra que o código passa a afirmar é uma só: nunca entregar um código que deixou de valer.

O prazo é uma coluna da linha, `expires_at`, nula por padrão, escrita por quem enfileira. O domínio que cunhou o código é o único que sabe quando ele morre, e ele já tem esse instante em mãos, que é o mesmo do cadastro pendente. Derivar o prazo dentro do outbox, a partir de `created_at` mais uma validade por tipo, repetiria a regra dos quinze minutos num segundo lugar, e num terceiro no dia em que a recuperação de senha chegar com uma validade diferente. Por isso o cadastro e o reenvio passam a expiração do próprio cadastro pendente, e o aviso de senha alterada passa nulo, porque um aviso não tem prazo e a política de tentativas já o limita.

Antes de enviar, o worker compara o prazo com o relógio, e a fronteira é estrita: passou do prazo significa depois dele, não no instante dele, então uma mensagem cujo conteúdo morre exatamente agora ainda sai. Passada a fronteira, a linha recebe o mesmo tratamento terminal da desistência, com o conteúdo limpo e `next_attempt_at` marcando a transição, e o handler de desistência roda. Isso importa: a pessoa não recebeu o código, então ela não pode pagar por ele, e a cota de reenvio volta para ela.

Vencer, porém, não é falhar, e por isso `expired` é um status próprio e não um `failed`. `failed` quer dizer que o provedor recusou, o que é problema de integração, enquanto `expired` quer dizer que demoramos demais, o que é problema de disponibilidade nossa. Juntar os dois no mesmo balde apagaria justamente o sinal que contaria que o worker passou tempo fora do ar. Como a coluna é texto livre, isso não custa migration nenhuma, e a retenção trata os dois do mesmo jeito.

A substituição é morta no enfileiramento, pelo `auth`, dentro da mesma transação. Quando o domínio enfileira um código novo para um destinatário, as linhas daquele mesmo tipo e daquele mesmo destinatário que ainda estão pendentes são canceladas ali, junto da gravação que criou o código novo. O outbox expõe isso como uma operação genérica de cancelar as pendentes de um tipo para um destinatário, e quem decide chamá-la é o `auth`, porque a alternativa, que seria o worker perguntar ao domínio se aquele código ainda vale na hora de enviar, inverteria a dependência de mão única que a fatia inteira respeita.

Cancelar limpa o conteúdo, que é o que tira o código em claro do banco, e marca a linha como `canceled`, deixando que a retenção a remova depois pela janela das abandonadas. Apagar de uma vez também funcionaria para a exposição, mas levaria junto a evidência, e uma rajada de cancelamentos é exatamente o sintoma que se quer enxergar quando alguém está martelando o reenvio ou quando a entrega está atrasada.

O que o cancelamento não faz é devolver cota. A pessoa pediu o código novo e vai recebê-lo, então aquele envio foi consumido atendendo a ela. Devolver tornaria o teto decorativo, porque reenviar rápido o bastante, antes de a entrega anterior sair, impediria o contador de subir, que é precisamente o padrão de abuso contra o qual o teto existe. Vencimento e cancelamento são, portanto, dois caminhos terminais com efeitos opostos sobre a cota, e essa diferença é deliberada.

Por fim, a política de tentativas continua existindo ao lado do prazo, e vale a que disparar primeiro. As duas não são redundantes: o prazo compra correção, e as tentativas compram uma rendição rápida. Com o provedor fora do ar, desistir em cerca de quarenta segundos devolve a cota de imediato e libera a pessoa a pedir outro código, enquanto só o prazo martelaria um provedor morto por quinze minutos e a faria esperar todo esse tempo para ter a cota de volta.
...
* Tabela `email_outbox` criada por migration, com o índice composto de `status` e `next_attempt_at`, mais as colunas nulas `correlation_id` e `expires_at`, e nenhuma outra alteração de schema fora do escopo
* Porta `EmailOutboxRepository` com `enqueue`, `claimDue`, `markSent`, `reschedule`, `giveUp`, `expire` e `cancelPending`, adaptador Drizzle usando `FOR UPDATE SKIP LOCKED` na reivindicação e adaptador em memória espelhando a mesma semântica de lease
...
* Retenção rodando no fim de cada ciclo do worker, apagando a linha entregue depois de uma hora e a abandonada, seja ela `failed`, `expired` ou `canceled`, depois de trinta dias, com o conteúdo limpo já no instante do abandono, coberta por teste em cada janela, na linha pendente que nunca é tocada e na falha de expurgo que não impede a entrega do lote
* Mensagem com prazo próprio em `expires_at`, escrito pelo cadastro e pelo reenvio com a expiração do cadastro pendente e nulo no aviso de senha, com o worker recusando o envio apenas depois do prazo, marcando a linha `expired`, limpando o conteúdo e rodando o handler de desistência, coberto por teste nas três linhas em volta do instante do corte, na linha sem prazo que nunca vence e na desistência por tentativas que segue valendo quando o prazo está longe
* Cancelamento das linhas ainda pendentes do mesmo tipo e do mesmo destinatário na transação que enfileira o código novo, limpando o conteúdo e marcando `canceled`, coberto por teste no escopo que ele não toca, na ausência de compensação e na cota que não volta
